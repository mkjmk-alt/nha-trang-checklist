...
현재 `wrangler.jsonc`는 KV binding에 ID를 생략한 자동 프로비저닝 방식입니다. 첫 `npm run deploy` 때 Wrangler가 `CHECKLIST_KV`용 KV namespace를 만들고 설정에 연결합니다. 배포가 끝나면 다음과 같은 주소가 표시됩니다.
...
폴링 간격은 무료 사용량과 체감 속도를 고려해 3,000~5,000ms 사이로 자동 제한됩니다.
...
6. 상대 화면은 4초 이내에 GET 폴링으로 갱신됩니다. 앱을 백그라운드로 보내면 폴링을 멈추고, 돌아오면 즉시 최신 상태를 확인합니다.
