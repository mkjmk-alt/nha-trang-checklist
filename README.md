...
- 기본 수동 동기화, 변경 감지 후 3→5→10분 동안만 작동하는 가변 폴링
- 탭/앱이 백그라운드로 가면 폴링 중지, 복귀 시 한 번 동기화
...
페이지 진입·새로고침 시 최신 상태를 한 번 불러온 뒤 수동 모드로 전환합니다. 체크 변경은 서버에 즉시 저장되며, 그때부터 3분, 5분, 10분 뒤에 차례로 상태를 확인한 다음 다시 수동 모드로 돌아갑니다. 수동 동기화나 화면 복귀 확인에서 상대방의 새 변경을 받으면 3분 단계부터 다시 시작합니다.
...
6. 체크 변경이나 상대방의 새 변경이 발견되면 3분, 5분, 10분 뒤에 자동 확인하고 다시 수동 모드로 전환됩니다. 앱을 백그라운드로 보내면 예약을 멈추고, 돌아오거나 새로고침하면 최신 상태를 한 번 확인합니다.
7. 인터넷이 끊기면 로컬에 계속 저장합니다. 연결이 돌아오면 항목별 `updatedAt`을 비교해 더 최신인 변경을 서버로 보냅니다.

```text
내 휴대폰 ── 체크 즉시 PUT ──▶ Cloudflare Worker ──▶ room별 KV JSON
     ▲                                      │
     └──── 변경 감지 후 3→5→10분 GET, 이후 수동 ────┘
                                            ▲
동행자 휴대폰 ───────────────────────────────┘
```
